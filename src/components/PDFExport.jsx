...
<body>
  ...
  <table>
    ...
    ${Object.keys(bySource).map(s => `<tr><td>${sourceNames[s] || s}</td><td class="amt">Rs.${bySource[s].toLocaleString('en-IN')}</td></tr>`).join('')}
  ...
  <table>
    ...
    ${Object.keys(byCat).length === 0 ? '<tr><td colspan="2" style="text-align:center;color:#999;padding:12px">Koi kharcha nahi hua iss mahine</td></tr>' :
      Object.keys(byCat).map(c => `<tr><td>${catNames[c] || c}</td><td class="amt">Rs.${byCat[c].toLocaleString('en-IN')}</td></tr>`).join('')}
  ...
  <table>
    ...
    ${memberStatus.map((m, i) => `<tr><td>${i + 1}</td><td><strong>${m.name}</strong></td>
      <td>${m.paid
  ...
  <table>
    ...
    ${collections.length === 0
      ...
      : collections.map((c, i) => `<tr><td>${i + 1}</td><td>${c.date || '-'}</td><td>${c.memberName || '-'}</td><td class="amt">Rs.${Number(c.amount).toLocaleString('en-IN')}</td><td>${sourceNames[c.source] || c.source || '-'}</td><td>${c.note || '-'}</td></tr>`).join('')}
  </table>
  ...
  <table>
    ...
    ${expenditure.length === 0
      ...
      : expenditure.map((e, i) => `<tr><td>${i + 1}</td><td>${e.date || '-'}</td><td>${catNames[e.category] || e.category || '-'}</td><td class="amt">Rs.${Number(e.amount).toLocaleString('en-IN')}</td><td>${e.description || '-'}</td></tr>`).join('')}
  </table>
