...
function DashboardSection() {
  const { members, collections, expenditure, settings } = useData();
...
function CollectionsSection() {
  ...
  const handleSave = () => {
    if (!form.memberId && !form.memberName) { alert('Please select a member or enter name!'); return; }
    if (!form.amount || Number(form.amount) <= 0) { alert('Please enter amount!'); return; }
    addCollection(form);
    setShowModal(false);
...
function ExpenditureSection() {
  ...
  const handleDelete = (id) => {
    if (window.confirm('Delete this expense?')) deleteExpenditure(id);
  };
...
function SettingsSection() {
  ...
  const handlePinChange = () => {
    if (form.pin !== settings.pin) { alert('Current PIN is wrong!'); return; }
    if (form.newPin.length !== 4) { alert('New PIN must be 4 digits!'); return; }
    if (form.newPin !== form.confirmPin) { alert('New PIN and Confirm PIN do not match!'); return; }
...
function NotificationsSection() {
  ...
  const handleSave = () => {
    ...
    if (!form.text.trim()) { alert('Message is required!'); return; }
