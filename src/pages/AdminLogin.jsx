...
function AdminLogin() {
  ...
  const isPinSet = settings.pin && settings.pin.length > 0;
  ...
  useEffect(() => {
    const isPinNowSet = settings.pin && settings.pin.length > 0;
    if (!isPinNowSet) {
      setMode('set');
      setStep(1);
    }
  ...
  const handlePinSubmit = () => {
    if (mode === 'set' && step === 1) {
      ...
      // Small timeout to let storage update, then switch to login mode
      setTimeout(() => {
        setMode('login');
        setStep(1);
        alert('PIN set successfully! Please login with your new PIN.');
      }, 50);
    } else if (mode === 'login') {
      if (pin.length !== 4) {
