...
import { enqueue, replayQueue, getPendingCount, clearQueue } from './utils/offlineQueue';
...
const validateCollection = (collection) => {
        ...
        const safeReceiptNo = String(collection?.receiptNo || '').trim().substring(0, 50);
...
export function DataProvider({ children }) {
        ...
        const saveAndVerify = useCallback(async (newData) => {
                ...
                if (!navigator.onLine) {
                        console.log('[SVAKS] Offline — queuing save for later');
                        enqueue({ type: 'save', payload: newData });
                        setSyncStatus('offline');
                        setSyncError('Offline — changes queued. Will sync when back online.');
                        setData(newData);
                        setSaving(false);
                ...
                if (!pushSuccess) {
                        // Push failed — queue it for retry
                        console.warn('[SVAKS] Push failed — queuing for retry');
                        enqueue({ type: 'save', payload: newData });
                        setSyncStatus('error');
                        setSyncError('Save queued — will retry when connection is stable.');
                        setData(newData);
                        setSaving(false);
        ...
        useEffect(() => {
                const handleOnline = async () => {
                        ...
                        // Step 1: Replay any queued saves
                        const pending = getPendingCount();
                        if (pending > 0) {
                                console.log(`[SVAKS] Replaying ${pending} queued saves...`);
                                const result = await replayQueue(async (payload) => {
                                        const ok = await pushToCloud(payload);
                                        return ok;
                                });
                                if (result.success > 0) {
                                        console.log(`[SVAKS] Replayed ${result.success} queued saves`);
                                }
                        }

                        // Step 2: Re-fetch latest from cloud
                ...
                const handleOffline = () => {
                        ...
                        setSyncStatus('offline');
                        setSyncError('Offline mode — changes will sync when back online.');
                };

                // Listen for background sync events from the service worker
                const handleBackgroundSync = () => {
                        console.log('[SVAKS] Background sync event from SW');
                        handleOnline();
                };

                window.addEventListener('online', handleOnline);
                window.addEventListener('offline', handleOffline);
                window.addEventListener('svaks-background-sync', handleBackgroundSync);

                return () => {
                        window.removeEventListener('online', handleOnline);
                        window.removeEventListener('offline', handleOffline);
                        window.removeEventListener('svaks-background-sync', handleBackgroundSync);
                };
        }, [fetchCloudData, applyCloudData, pushToCloud]);
